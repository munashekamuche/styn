...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (window.innerWidth > 768) {
    if (chats.length > 0 && isUserLoggedIn()) {
      selectChat(chats[0].id);
    }
  }
});

// Setup login check for all interactions
function setupLoginCheck() {
  ...
  // If not logged in, show modal on any interaction
  if (!isUserLoggedIn()) {
    // Disable inputs but allow viewing
    if (chatInput) {
      chatInput.placeholder = 'Sign in to send messages...';
    ...
    if (chatSearch) {
      chatSearch.addEventListener('focus', (e) => {
        e.target.blur();
        showLoginModal();
      });
    }
